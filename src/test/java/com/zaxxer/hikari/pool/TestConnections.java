...
public class TestConnections {
    ...
    @Test
    public void testHaisen() throws SQLException, InterruptedException {

        HikariConfig config = new HikariConfig();

        // 注意，这里 jdbcUrl 和 dataSourceClassName 二选一。
        config.setJdbcUrl("jdbc:mysql://rm-bp150s48920cjevsweo.mysql.rds.aliyuncs.com:3306/muse?useUnicode=true&characterEncoding=UTF-8&zeroDateTimeBehavior=convertToNull&useSSL=false&serverTimezone=Asia/Shanghai");
        // config.setDriverClassName("com.mysql.cj.jdbc.Driver");
        config.setUsername("haisen");
        config.setPassword("Haisen123");

        config.setMinimumIdle(1);
        config.setMaximumPoolSize(2);

        config.setConnectionTimeout(3000000);     // 默认：30s，等待建立连接的最大时长 (ms)
        config.setIdleTimeout(1800000);         // 默认：10m，一个连接空闲状态的最长时长 (ms)
         ...
         *
         * TODO：使用 jconsole 查看
         *  ![-w600](https://lhsaq2009.oss-cn-hangzhou.aliyuncs.com/image/202302150055888.png)
         */
        ...
        /*PreparedStatement preparedStatement = conn.prepareStatement("select * from user where id = 1");
